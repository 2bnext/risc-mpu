...
- **`toolchain/asm.py`** — two-pass assembler. Labels can be global (no leading dot) or local (`.foo`, scoped to the most recent global label). Reads `.asm`, writes `.mpu` binary. Documented in [toolchain/asm.md](toolchain/asm.md).
  - **Pseudo-instructions**: `jmp target` → `beq.32 r0,#0,target`; `mov rD, rS` → `ld.32 rD, rS`; `pop rN` → `ld.32 rN, [sp+=4]`; `push rN` → `sub.32 sp,#4 ; st.32 [sp], rN` (the only multi-word pseudo, expanded by `expand_pseudos()` before pass 1). Labels may be attached to a `push` line.
